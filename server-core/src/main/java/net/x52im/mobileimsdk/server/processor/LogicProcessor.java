...
public class LogicProcessor {
    ...
    public void processLogin(final Channel session, final Protocol pFromClient, final String remoteAddress) throws Exception {
        ...
        if (serverCoreHandler.getServerEventListener() != null) {
            ...
            if (alreadyLogined) {
                ...
                processLoginSuccessSend(session, loginInfo, remoteAddress);
            } else {
    ...
    private void processLoginSuccessSend(final Channel session, final PLoginInfo loginInfo, final String remoteAddress) throws Exception {
        ...
        MBObserver sendResultObserver = (sendOK, extraObj) -> {
            if (sendOK) {
                ...
                if (putOK) {
                    ...
                    OnlineProcessor.setUserTokenForChannel(session, loginInfo.getLoginToken());
                    OnlineProcessor.setFirstLoginTimeForChannel(session, firstLoginTimeToClient);
                    serverCoreHandler.getServerEventListener().onUserLoginSuccess(loginInfo.getLoginUserId(), loginInfo.getExtra(), session);
                }
            } else {
